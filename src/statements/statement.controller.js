...
export const generateMonthlyStatements = async () => {
  ...
  try {
    ...
    const mesAnterior =
      fechaActual.getMonth() === 0 ? 12 : fechaActual.getMonth();
    const anioAnterior =
    ...
    await t.commit();
    console.log(
      `[Cierre] Estados de cuenta generados para el periodo ${mesAnterior}/${anioAnterior}`,
    );
  } catch (error) {
    await t.rollback();
    console.error("Error al generar estados de cuenta:", error);
  }
};
