  ...
  try {
    ...
    for (const account of accounts) {
      const montoInteres = parseFloat(
        (account.saldo * (TASA_ANUAL / 12)).toFixed(2),
      );
      ...
      // 1. Definimos la operación (INTERESES es una entrada, por lo tanto un CRÉDITO)
      const tipoOperacion = "INTERESES";
      const esEgreso = ["RETIRO", "TRANSFERENCIA", "PAGO_PRESTAMO"].includes(
        tipoOperacion,
      );

      // 2. Registro del movimiento dinámico
      await Movement.create(
        {
          tipo_operacion: tipoOperacion,
          tipo_movimiento: esEgreso ? "DEBITO" : "CREDITO", // Se calculará como CRÉDITO
          monto: montoInteres,
