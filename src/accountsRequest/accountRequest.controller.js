...
export const approveRequest = async (req, res) => {
  try {
    ...
    const monetariaCount = await Account.count({
      where: {
        user_id: request.user_id,
        tipo: ACCOUNT_TYPES.CHECKING,
      },
    });

    // 🔹 Validar límites
    if (request.tipo === ACCOUNT_TYPES.SAVINGS && ahorroCount >= 2) {
      return res.status(400).json({
        message: "El usuario ya tiene el máximo de 2 cuentas de ahorro.",
      });
    }

    if (request.tipo === ACCOUNT_TYPES.CHECKING && monetariaCount >= 1) {
      return res.status(400).json({
